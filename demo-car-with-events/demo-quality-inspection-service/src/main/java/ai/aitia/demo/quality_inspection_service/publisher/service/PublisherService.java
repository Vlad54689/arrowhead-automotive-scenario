package ai.aitia.demo.quality_inspection_service.publisher.service;
...
public class PublisherService {
	...
	public void publish(final PresetEventType eventType, final Map<String, String> metadata, final String payload) {
		...
		logger.info("Publishing event '{}' to the Event Handler with payload: {}", eventType.getEventTypeName(), payload);
		try {
			arrowheadService.publishToEventHandler(request);
			logger.info("Successfully published event '{}' to the Event Handler.", eventType.getEventTypeName());
		} catch (final Exception ex) {
			logger.error("Failed to publish event '{}' to the Event Handler: {}", eventType.getEventTypeName(), ex.getMessage(), ex);
			throw ex;
		}
	}
